$(function() {
	...
	let selfile = null;
	// 이미지파일 수정
	$('#btn-more-sell').click(function(event) {
	...
	function moremysell() {
		...
		$.ajax({
			url: '/mingle/mypage/myselllist/add', // 실제 URL로 변경 필요
			type: 'GET',
			data: boardLastIndex,
			success: function(response) {
				...
				var newHtml = response.actInfos.map(function(actInfo) {
					var statusDiv = actInfo.selldone ? '<div class="div-soldout"></div>' : '<div class="div-forsale"></div>';
